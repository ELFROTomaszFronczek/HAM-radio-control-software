            ...
            public string getCountryName
            {
                get
                {
                    if (string.IsNullOrEmpty(country))
                    {
                        ...
                        country = Countries.getCountryNameByCallSign(callSign);
                ...
                if (element.IndexOf(":") > -1)
                {
                    ...
                    if (el.Length > 1)
                    {
                        ...
                        switch (n)
                        {
                            ...
                            case "datetime": spot.dateTime = val; break;
        ...
        private static string fixElement(string v)
        {
            ...
            if (el != null)
            {
                if (el.Length > 10)
                {
                    ...
                    if (el.Length > 1) v = "dateTime:" + toDate(el[0].ToUpper()) + " " + toTime(el[1]) + "\r\n";
                    if (el.Length > 2) v += "frequency:" + trymuj(el[2]) + "\r\n";
                    if (el.Length > 3) v += "callSign:" + removeAspace(trymuj(el[3])) + "\r\n";

                    if (el.Length > 4) v += trymuj(el[4]) + "\r\n";
                    if (el.Length > 5) v += trymuj(el[5]) + "\r\n";
                    if (el.Length > 6) v += trymuj(el[6]) + "\r\n";
                    if (el.Length > 7) v += trymuj(el[7]) + "\r\n";
                    if (el.Length > 8) v += trymuj(el[8]) + "\r\n";
                    if (el.Length > 9) v += trymuj(el[9]) + "\r\n";
                    if (el.Length < 12)
                    {
                        if (el.Length > 10) v += "spotter:" + removeAspace(trymuj(el[10])) + "\r\n";
                        if (el.Length > 10) v += "comments:" + removeBspace(trymuj(el[10])) + "\r\n";
                    }
                    else
                    {
                        if (el.Length > 10) v += "spotter:" + trymuj(el[10]) + "\r\n";
                        if (el.Length > 11) v += "comments:" + trymuj(el[11]) + "\r\n";
                    }
                ...
                else
                {
                    ...
                    if (el.Length < 7)
                    {
                        if (el.Length > 5) v += "spotter:" + removeAspace(trymuj(el[5])) + "\r\n";
                        if (el.Length > 5) v += "comments:" + removeBspace(trymuj(el[5])) + "\r\n";
                    }
                    else
                    {
                        if (el.Length > 5) v += "spotter:" + trymuj(el[5]) + "\r\n";
                        if (el.Length > 6) v += "comments:" + trymuj(el[6]) + "\r\n";
                    }
                }
            }
